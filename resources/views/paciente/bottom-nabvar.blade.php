...
@php
  ...
  $__items  = Notificacion::where('fkUsuario', $__uid)
              ->orderBy('fecha','desc')
              ->limit(10)->get();
@endphp
...
<style>
  ...
  .patient-bottom-navbar .pbn-link:hover{ color:#111827; }
  ...
  /* ===== Modal “bottom sheet” azul: estilos scoped (no requieren Bootstrap CSS) ===== */
  ...
  .modal-title{
    margin:0; font-size:1rem; font-weight:800; color:#1e40af; /* azul intenso */
  }
  .close{
    padding:.25rem .5rem; background:transparent; border:0; font-size:1.25rem; line-height:1;
    color:#334155;
  }
  /* Cuerpo bajito y scrollable */
  .modal-body{ padding:0; }
  .modal-dialog-scrollable .modal-body{
    max-height: 42vh; /* bajito */
    overflow-y:auto;
  }

  /* Backdrop gris (oscurece fondo) */
  ...
  /* Lista con toques azules */
  .list-group{ list-style:none; margin:0; padding:0; }
  .list-group-item{
    padding:.75rem .95rem; border-bottom:1px solid #eff2f8; background:#ffffff;
  }
  ...
  /* Animación bottom-sheet (entrada suave) */
...
<div class="patient-bottom-navbar" role="navigation" aria-label="Barra inferior de paciente">
  <div class="pbn-wrap">
    ...
    {{-- Campana: abre modal --}}
    <a href="#" class="pbn-link" data-toggle="modal" data-target="#pbnNotiModal" aria-label="Notificaciones">
      <i class="fas fa-bell fa-lg"></i>
      <span>Notificaciones</span>
      <span class="pbn-badge" id="pbn-noti-badge">{{ $__unread > 99 ? '99+' : $__unread }}</span>
    </a>

    <a href="{{ url('/paciente/dashboard') }}" class="pbn-link" aria-label="Inicio">
      <i class="fas fa-home fa-lg"></i>
      <span>Inicio</span>
    </a>

    <a href="{{ url('/paciente/actividades') }}" class="pbn-link" aria-label="Actividades">
